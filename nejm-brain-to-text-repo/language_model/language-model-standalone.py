...
# Import Llama rescorer (optional - falls back to OPT if not available)
try:
...
def main(args):
    ...
    # initialize LLM model (OPT or Llama)
    model_type = args.model_type
    use_llm = do_opt or model_type == 'llama'

    if use_llm:
        if model_type == 'llama' and LLAMA_AVAILABLE:
            logging.info(f"Building Llama model: {args.llama_model}...")
            ...
            lm, lm_tokenizer = build_llama(
                ...
                adapter_path=args.llama_adapter,
            )
            logging.info(f'Llama model successfully built in {(time.time()-start_time):0.4f} seconds.')
            rescore_fn = rescore_with_llama
        else:
            # Fall back to OPT
            if model_type == 'llama' and not LLAMA_AVAILABLE:
                logging.warning("Llama rescorer not available. Falling back to OPT.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', type=str, default='opt', choices=['opt', 'llama'],
                        help='LLM type to use for rescoring: opt or llama')
    parser.add_argument('--llama_model', type=str, default='meta-llama/Llama-3.1-8B',
                        help='HuggingFace model name for Llama')
    parser.add_argument('--llama_adapter', type=str, default=None,
                        help='Path to QLoRA adapter for Llama (optional)')
    parser.add_argument('--use_4bit', action='store_true', default=True,
                        help='Use 4-bit quantization for Llama')
