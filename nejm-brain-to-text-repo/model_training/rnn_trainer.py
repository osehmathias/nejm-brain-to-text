...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        model_type = self.args['model'].get('type', 'gru')
        ...
        if model_type == 'conformer':
            self.model = ConformerDecoder(
                ...
                d_model=self.args['model'].get('d_model', 768),
                n_heads=self.args['model'].get('n_heads', 8),
                d_ff=self.args['model'].get('d_ff', 3072),
                n_layers=self.args['model'].get('n_layers', 6),
                conv_kernel_size=self.args['model'].get('conv_kernel_size', 31),
                dropout=self.args['model'].get('conformer_dropout', 0.1),
        ...
        # Set encoder and/or input layers to not trainable if specified
        encoder_trainable = self.args['model'].get('rnn_trainable', True)
        for name, param in self.model.named_parameters():
            # For GRU model, check for 'gru'; for Conformer, check for 'conformer' or other encoder parts
            is_encoder_param = 'gru' in name or 'conformer' in name or 'attn' in name or 'conv' in name or 'ff' in name
            if not encoder_trainable and is_encoder_param:
    ...
    def create_optimizer(self):
        ...
        '''
        # Collect bias parameters (no weight decay)
        bias_params = [p for name, p in self.model.named_parameters()
                       if 'bias' in name and 'day' not in name]
        # Day-specific parameters (separate learning rate)
