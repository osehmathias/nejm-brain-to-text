...
class BrainToTextDecoder_Trainer_Attention:
    """
    This class will initialize and train a brain-to-text phoneme decoder with attention

    Based on BrainToTextDecoder_Trainer but uses GRUDecoderWithAttention
    """
    ...
    def __init__(self, args):
        ...
        # Create output directory - append _attention to distinguish from baseline
        if args['mode'] == 'train':
        ...
        # Get attention-specific parameters with defaults
        attention_config = self.args.get('attention', {})
        n_attention_heads = attention_config.get('n_heads', 8)
        attention_dropout = attention_config.get('dropout', 0.1)
        attention_layers = attention_config.get('n_layers', 1)

        # Initialize the model with attention
        self.model = GRUDecoderWithAttention(
        ...
        self.logger.info(f"Initialized RNN+Attention decoding model")
        self.logger.info(f"Attention config: {n_attention_heads} heads, {attention_layers} layers, {attention_dropout} dropout")
        ...
        attention_params = 0
        for name, param in self.model.named_parameters():
            if 'attention' in name or 'norm' in name or 'ffn' in name:
                attention_params += param.numel()
        self.logger.info(f"Model has {attention_params:,} attention-related parameters | {((attention_params / total_params) * 100):.2f}% of total parameters")
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            with torch.autocast(device_type = self.device.type, enabled = self.args['use_amp'], dtype = torch.bfloat16):
