#!/usr/bin/env python3
"""
Run inference on test set using fine-tuned OpenAI model.

Usage:
    python run_inference.py --model ft:gpt-4.1-2025-04-14:org::phoneme-predict:xxxxx
    python run_inference.py --model ft:gpt-4.1-2025-04-14:org::phoneme-predict:xxxxx --input rnn_baseline/finetune_test.jsonl
"""
...
def load_existing_results(csv_path: str) -> set[str]:
    """Load already-processed phoneme inputs from existing results file."""
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            # Use phonemes as unique key (they're unique per sample)
            processed.add(row['phonemes'])
...
def run_inference(client: OpenAI, model: str, samples: list[dict], max_retries: int = 10) -> list[dict]:
    ...
    for sample in tqdm(samples, desc="Running inference"):
        ...
        messages.append({"role": "user", "content": sample['phonemes']})
        ...
        for attempt in range(max_retries):
            try:
                ...
                base_wait = min(60, 2 ** attempt)  # Cap at 60s
...
def main():
    ...
    parser.add_argument('--model', type=str, required=True,
                        help='Fine-tuned model ID (e.g., ft:gpt-4.1-2025-04-14:org::phoneme-predict:xxxxx)')
    parser.add_argument('--input', type=str, default='rnn_baseline/finetune_test.jsonl',
                        help='Input JSONL file')
    parser.add_argument('--output', type=str, default='rnn_baseline/inference_results.csv',
    ...
    if processed:
        ...
        samples = [s for s in all_samples if s['phonemes'] not in processed]
    ...
    # Save results (append if file exists, otherwise create with header)
    print(f"\nSaving results to {args.output}...")
    Path(args.output).parent.mkdir(parents=True, exist_ok=True)

    file_exists = Path(args.output).exists() and len(processed) > 0
    fieldnames = ['phonemes', 'prediction', 'ground_truth', 'time_ms', 'tokens_in', 'tokens_out']
    ...
    cost_in = total_tokens_in / 1_000_000 * 2.00  # $2/1M input
    cost_out = total_tokens_out / 1_000_000 * 8.00  # $8/1M output
