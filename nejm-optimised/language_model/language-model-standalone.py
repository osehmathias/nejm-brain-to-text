...
# Import Llama rescorer for improved WER
from llama_rescorer import build_llama, rescore_with_llama, llama_lm_decode
...
def main(args):
    ...
    do_llama = args.do_llama      # Use Llama 3.1 8B instead of OPT
    ...
    # If both do_opt and do_llama are set, prefer Llama
    if do_opt and do_llama:
        logging.warning("Both --do_opt and --do_llama are set. Using Llama (recommended).")
        do_opt = False
    ...
    # initialize LLM model (Llama or OPT)
    lm = None
    lm_tokenizer = None

    if do_llama:
        logging.info(f"Building Llama model: {llama_model}...")
        start_time = time.time()
        lm, lm_tokenizer = build_llama(
            model_name=llama_model,
            cache_dir=llama_cache_dir,
            device=str(device),
            use_4bit=True,
            adapter_path=llama_adapter_path,
        )
        logging.info(f'Llama model successfully built in {(time.time()-start_time):0.4f} seconds.')
    elif do_opt:
    ...
    while True:
        ...
        try:
            ...
            if len(lm_finalize_stream) > 0:
                ...
                # Optionally rescore with a LLM (Llama or OPT)
                if do_llama:
                    startT = time.time()

                    decoded_final, nbest_redis, confidences = llama_lm_decode(
                        lm,
                        lm_tokenizer,
                        str(device),
                        nbest_out,
                    ...
                    logging.info('Llama time: %.3f' % (time.time() - startT))
            ...
            if len(update_params_stream) > 0:
                for entry_id, entry_data in update_params_stream[0][1]:
                    ...
                    do_llama = int(entry_data.get(b'do_llama', do_llama))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--do_llama', action='store_true', help='Use Llama 3.1 8B for rescoring (recommended over OPT)')
    parser.add_argument('--llama_model', type=str, default='meta-llama/Llama-3.1-8B-Instruct', help='Llama model name')
    parser.add_argument('--llama_cache_dir', type=str, default=None, help='path to Llama cache')
    parser.add_argument('--llama_adapter_path', type=str, default=None, help='path to QLoRA adapter (optional)')
    parser.add_argument('--alpha', type=float, default=0.5, help='alpha value [0-1]: Higher = more weight on LLM rescore. Lower = more weight on ngram rescore')
